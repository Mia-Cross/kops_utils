...
go run -v ./cmd/kops -v10 create cluster --cloud=scaleway --zones=nl-ams-1 --name="$CLUSTER_NAME" --networking=calico --yes
if [ $? != 0 ]; then
  echo "ERROR CREATING CLUSTER"
  exit
fi
go run -v ./cmd/kops go run -v ./cmd/kops edit ig control-plane-nl-ams-1
if [ $? != 0 ]; then
